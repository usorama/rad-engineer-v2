 ...
 *
 * Tests:
 * - getAllTasks returns dummy tasks
...
describe("ElectronIPCAdapter: getAllTasks", () => {
  it("Returns 2 dummy tasks on initialization", async () => {
    ...
    // Validate we have exactly 2 dummy tasks
    expect(tasks).toHaveLength(2);

    // Validate first task
    expect(tasks[0].id).toBe("demo-1");
    expect(tasks[0].title).toBe("Demo Task 1");
    expect(tasks[0].status).toBe("pending");
    expect(tasks[0].priority).toBe(4);
    expect(tasks[0].tags).toEqual(["demo", "ui-integration"]);

    // Validate second task
    expect(tasks[1].id).toBe("demo-2");
    expect(tasks[1].title).toBe("Demo Task 2");
    expect(tasks[1].status).toBe("completed");
    expect(tasks[1].priority).toBe(3);
    expect(tasks[1].tags).toEqual(["demo", "completed"]);
  ...
  it("Returns tasks sorted by creation time (newest first)", async () => {
    ...
    // Create new task (will be newest)
...
describe("ElectronIPCAdapter: createTask", () => {
  it("Creates task with generated unique ID", async () => {
    ...
    // Validate task has unique ID with correct format
...
describe("ElectronIPCAdapter: startTask", () => {
  it("Starts task and emits progress events", async () => {
    ...
    // Validate all events have correct taskId
  ...
  it("Throws error for non-existent task", async () => {
    ...
    // Attempt to start non-existent task
  ...
  it("Throws error when starting already in-progress task", async () => {
    ...
    // Start task (don't await to keep it in progress)
    const startPromise = adapter.startTask(task.id);

    // Wait a bit for status to update to in_progress
  ...
  it("Throws error when starting already completed task", async () => {
    ...
    // Use pre-existing completed dummy task
    const completedTaskId = "demo-2";

    // Attempt to start completed task
    await expect(adapter.startTask(completedTaskId)).rejects.toThrow(
      "Task already completed"
    );
  });

  it("Logs WaveOrchestrator stub call", async () => {
    const adapter = new ElectronIPCAdapter({
      projectDir: "/test/project",
    });
    ...
    // Validate WaveOrchestrator stub was logged
    const stubLog = logs.find((log) =>
      log.includes("WaveOrchestrator stub called")
    );
    expect(stubLog).toBeTruthy();
    expect(stubLog).toContain(task.id);
  });
});
...
describe("ElectronIPCAdapter: getProjectDir", () => {
  it("Returns configured project directory", () => {
    const projectDir = "/test/project/path";
    const adapter = new ElectronIPCAdapter({
      projectDir,
    });
...
describe("ElectronIPCAdapter: debug mode", () => {
  it("Logs debug messages when debug enabled", async () => {
    ...
    // Validate debug logs were emitted
