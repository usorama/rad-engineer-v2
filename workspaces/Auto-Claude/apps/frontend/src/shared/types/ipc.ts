...
export interface ElectronAPI {
  ...
  // rad-engineer source environment operations
  ...
  // Execution monitoring operations (wave-based atomic swarm execution)
  execution: {
    getWaveStatus: (specId: string) => Promise<IPCResult<{
      waves: Array<{
        id: string;
        name: string;
    ...
    onWaveProgress: (specId: string, callback: (wave: {
      id: string;
      name: string;
      status: 'pending' | 'running' | 'completed' | 'failed';
      agentCount: number;
    ...
    getAgentStatus: (specId: string) => Promise<IPCResult<{
      agents: Array<{
        id: string;
        name: string;
    ...
    onAgentProgress: (specId: string, callback: (agent: {
      id: string;
      name: string;
      status: 'running' | 'completed' | 'failed' | 'idle';
      cpuPercent: number;
    ...
    getQualityGates: (specId: string) => Promise<IPCResult<Array<{
      type: 'typecheck' | 'lint' | 'test';
      status: 'pending' | 'running' | 'passed' | 'failed';
      duration: number;
      output?: string;
    }>>>;
    onQualityGateResult: (specId: string, callback: (result: {
      type: 'typecheck' | 'lint' | 'test';
      status: 'pending' | 'running' | 'passed' | 'failed';
      duration: number;
      output?: string;
    }) => void) => () => void;
    getStateMachineStatus: (specId: string) => Promise<IPCResult<{
      states: Array<{
        id: string;
        name: string;
    ...
    onStateChanged: (specId: string, callback: (stateChange: {
      fromState: string;
      toState: string;
      timestamp: string;
    }) => void) => () => void;
