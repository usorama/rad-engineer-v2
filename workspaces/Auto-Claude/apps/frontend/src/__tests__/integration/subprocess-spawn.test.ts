...
const RAD_ENGINEER_SOURCE = path.join(TEST_DIR, 'auto-claude-source');
...
function setupTestDirs(): void {
  ...
  mkdirSync(RAD_ENGINEER_SOURCE, { recursive: true });

  // Create runners subdirectory with spec_runner.py marker (used by getAutoBuildSourcePath)
  mkdirSync(path.join(RAD_ENGINEER_SOURCE, 'runners'), { recursive: true });

  // Create mock spec_runner.py in runners/ subdirectory (used as backend marker)
  writeFileSync(
    path.join(RAD_ENGINEER_SOURCE, 'runners', 'spec_runner.py'),
    '# Mock spec runner\nprint("Starting spec creation")'
  );
  // Create mock run.py
  writeFileSync(
    path.join(RAD_ENGINEER_SOURCE, 'run.py'),
...
describe('Subprocess Spawn Integration', () => {
  ...
  describe('AgentManager', () => {
    it('should spawn Python process for spec creation', async () => {
      ...
      manager.configure(undefined, RAD_ENGINEER_SOURCE);
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: RAD_ENGINEER_SOURCE,  // Process runs from auto-claude source directory
    ...
    it('should use configured Python path', async () => {
      ...
      const manager = new AgentManager();
      manager.configure('/custom/python3', RAD_ENGINEER_SOURCE);
