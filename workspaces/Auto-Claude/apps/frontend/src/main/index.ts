...
app.setName('rad-engineer');
if (process.platform === 'darwin') {
  // Force the name to appear in dock on macOS
  app.name = 'rad-engineer';
}
...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId('com.radengineer.ui');
...
app.on('before-quit', async () => {
  ...
  // Cleanup rad-engineer adapter
  const { cleanupRadEngineerAdapter } = await import('./ipc-handlers/rad-engineer-handlers');
  cleanupRadEngineerAdapter();
  console.warn('[main] Rad-engineer adapter cleaned up');
});
