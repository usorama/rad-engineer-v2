...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect rad-engineer source path. Please configure manually in settings.');
...
export function registerSettingsHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<AppSettings>> => {
      ...
      // See: https://github.com/AndyMik90/rad-engineer/issues/XXX
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# rad-engineer Framework Environment Variables',
          '# Managed by rad-engineer UI',
          '',
