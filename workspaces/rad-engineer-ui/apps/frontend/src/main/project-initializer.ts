...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (existsSync(gitignorePath)) {
    ...
    writeFileSync(gitignorePath, '# rad-engineer data directory\n' + entriesToAdd.join('\n') + '\n');
  }
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  // Check git status - rad-engineer requires git for worktree-based builds
  const gitStatus = checkGitStatus(projectPath);
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    debug('Git check failed', { gitStatus });
    return {
      success: false,
      error: gitStatus.error || 'Git repository required. rad-engineer uses git worktrees for isolated builds.'
    };
  }
 ...
 * not an installation. This allows rad-engineer to be used to develop itself.
 */
